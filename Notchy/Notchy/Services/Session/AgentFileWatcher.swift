...
private let agentWatcherLogger = Logger(subsystem: "com.notchy.app", category: "AgentFileWatcher")

/// Protocol for receiving agent file update notifications
protocol AgentFileWatcherDelegate: AnyObject, Sendable {
...
class AgentFileWatcher {
    ...
    private let queue = DispatchQueue(label: "com.notchy.agentfilewatcher", qos: .userInitiated)
    ...
    private func startWatching() {
        ...
        guard FileManager.default.fileExists(atPath: filePath),
            ...
            agentWatcherLogger.warning("Failed to open agent file: \(self.filePath, privacy: .public)")
            return
        }
        ...
        do {
            ...
            agentWatcherLogger.error("Failed to seek to end: \(error.localizedDescription, privacy: .public)")
            return
        }
        ...
        agentWatcherLogger.debug("Started watching agent file: \(self.agentId.prefix(8), privacy: .public) for task: \(self.taskToolId.prefix(12), privacy: .public)")
    }
    ...
    private func parseTools() {
        ...
        agentWatcherLogger.debug("Agent \(self.agentId.prefix(8), privacy: .public) has \(tools.count) tools")

        let sid = self.sessionId
        let tid = self.taskToolId
        let del = self.delegate
        DispatchQueue.main.async {
            del?.didUpdateAgentTools(
                sessionId: sid,
                taskToolId: tid,
                tools: tools
            )
    ...
    private func stopInternal() {
        if source != nil {
            agentWatcherLogger.debug("Stopped watching agent file: \(self.agentId.prefix(8), privacy: .public)")
        }
        source?.cancel()
        source = nil
    }
...
class AgentFileWatcherManager {
    ...
    func startWatching(sessionId: String, taskToolId: String, agentId: String, cwd: String) {
        ...
        agentWatcherLogger.info("Started agent watcher for task \(taskToolId.prefix(12), privacy: .public)")
    }
...
class AgentFileWatcherBridge: AgentFileWatcherDelegate {
    ...
    nonisolated func didUpdateAgentTools(sessionId: String, taskToolId: String, tools: [SubagentToolInfo]) {
        Task {
