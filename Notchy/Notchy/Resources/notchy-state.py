...
"""
Notchy Hook
- Sends session state to Notchy.app via Unix socket
...
NOTCHY_VERSION = "1.0.0"
SOCKET_PATH = os.path.join(os.environ.get('TMPDIR', '/tmp'), 'notchy.sock')
...
def send_event(state):
    ...
    try:
        ...
        print(f"notchy: unexpected error: {e}", file=sys.stderr)
        sys.exit(1)
...
def main():
    try:
        ...
        print("notchy: failed to parse JSON from stdin", file=sys.stderr)
        sys.exit(1)
            ...
            if decision == "allow":
                ...
                output = {
                    "hookSpecificOutput": {
                        ...
                        "decision": {
                            "behavior": "deny",
                            "message": reason or "Denied by user via Notchy",
                        },
                    }
                }
                print(json.dumps(output))
