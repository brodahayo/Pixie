...
actor ConversationParser {
    nonisolated(unsafe) static let shared = ConversationParser()

    nonisolated static let logger = Logger(subsystem: "com.notchy.app", category: "Parser")
    ...
    // MARK: - Conversation Info Parsing
    ...
    struct IncrementalParseResult: Sendable {
    ...
    private static func parseStructuredResult(
        ...
        if toolName.hasPrefix("mcp__") {
            ...
            var rawResult: [String: String] = [:]
            for (key, value) in toolUseResult {
